...
def make_bars(closes, opens=None, highs=None, lows=None) -> pd.DataFrame:
    n = len(closes)
...
def chart_bb_reversal_lower():
    ...
    plot_candles(
        df,
        title="bb_reversal (side=lower) — prev 跌出下軌, cur 反向 K 收回軌道內",
        overlays={
...
def chart_bb_reversal_upper():
    ...
    plot_candles(
        df,
        title="bb_reversal (side=upper) — prev 突破上軌, cur 反向 K 收回軌道內",
        overlays={
            "BB upper": (bb["upper"], "#888888"),
            "BB middle": (bb["middle"], "#444444"),
            "BB lower": (bb["lower"], "#888888"),
...
def chart_range_breakout_up():
    ...
    df = make_bars(closes, opens=opens, highs=highs, lows=lows)
    ...
    plot_candles(
        df,
        title="range_breakout (direction=up) — close 突破前 20 根 high 最高點",
...
def chart_range_breakout_down():
    ...
    plot_candles(
        df,
        title="range_breakout (direction=down) — close 跌破前 20 根 low 最低點",
